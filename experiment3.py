        ...
        for random_state in random_states:
            ...
            for i_metric, metric_name in enumerate(metric_names):
                for j_clf, clf_name in enumerate(clf_names):
                    ...
                    filename = "results/experiment3/metrics/gen/%s/%s/%s/%s.csv" % (drift, stream_name, metric_name, clf_name)
                    if not os.path.exists("results/experiment3/metrics/gen/%s/%s/%s/" % (drift, stream_name, metric_name)):
                        os.makedirs("results/experiment3/metrics/gen/%s/%s/%s/" % (drift, stream_name, metric_name))
