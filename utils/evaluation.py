...
class Evaluation(threading.Thread):
    ...
    def compute_metrics(self):
        ...
        self.__metric_names = ",accuracy,balanced_accuracy,recall,precision,cohen_cappa,cappa_m,matthews_corrcoef,harmonic_mean,fbeta_score,auc,f1_score,gmean"
        ...
        for i in range(0, int(step)):
            ...
            gmean_score += [geometric_mean_score(y_true, y_pred, average="binary")]
            try:
                auc += [metrics.roc_auc_score(y_true, predict_probas[:,1])]
            except ValueError:
                auc += [0]
        ...
        self.__gmean_score = np.array(gmean_score)
